...
public final class ScanService extends Service {
    ...
    private void runScanner(double threshold)throws Exception{
        ...
        Process p=new ProcessBuilder("su","-c",cmd).redirectErrorStream(true).start();scanProcess=p;
    ...
    private void requestStop(String why){status(why);center("STOPPING SCRIPT...",900);worker.execute(()->{try{execRoot("touch "+q(STOP)+"; sleep 1; pkill -f 'nxlab-runner scan' 2>/dev/null || true",4000);}catch(Exception ignored){}Process p=scanProcess;if(p!=null)p.destroy();});}
    ...
    private String execRoot(String cmd,long timeout)throws Exception{Process p=new ProcessBuilder("su","-c",cmd).redirectErrorStream(true).start();ByteArrayOutputStream o=new ByteArrayOutputStream();Thread r=drain(p.getInputStream(),o);waitFor(p,timeout);r.join(500);return o.toString("UTF-8");}
    private void pipeToRoot(String cmd,InputStream src,long timeout)throws Exception{Process p=new ProcessBuilder("su","-c",cmd).redirectErrorStream(true).start();Thread r=drain(p.getInputStream(),new ByteArrayOutputStream());Thread w=new Thread(()->{try(OutputStream out=p.getOutputStream()){byte[] b=new byte[65536];int n;while((n=src.read(b))>0)out.write(b,0,n);out.flush();}catch(Exception ignored){}});w.start();waitFor(p,timeout);w.join(1000);r.join(500);if(p.exitValue()!=0)throw new IOException("root stage exit="+p.exitValue());}
    ...
    private void waitFor(Process p,long timeout)throws Exception{long d=SystemClock.uptimeMillis()+timeout;for(;;){try{p.exitValue();return;}catch(IllegalThreadStateException e){if(SystemClock.uptimeMillis()>d){p.destroy();throw new IOException("timeout");}Thread.sleep(40);}}}
