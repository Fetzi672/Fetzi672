...
public class LicenseOverlayService extends Service {
    ...
    private void requestServerRecheck(){
        ...
        new Thread(()->{
            try{
                ...
                JSONObject req=new JSONObject(); req.put("licenseKey",key); req.put("deviceId",DeviceIdentity.get(this)); req.put("appVersion",LicenseActivity.APP_VERSION);
                ...
                long anchor=SystemClock.elapsedRealtime(); getSharedPreferences(LicenseActivity.PREF,0).edit().putBoolean("verified",true).putString("expiry",exp).putLong("expiry_epoch_ms",expEpoch).putLong("server_epoch_ms",srvEpoch).putLong("verify_elapsed_ms",anchor).putInt("bound",bound).putInt("limit",limit).commit();
    ...
    private void enforceLock(final String label){
        if(locking)return; locking=true; getSharedPreferences(LicenseActivity.PREF,0).edit().putBoolean("verified",false).commit(); updateBadge(label);
